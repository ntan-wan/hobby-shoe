    ...
    return (
        <Card className={cn(productCardVariants(), className)} {...props} ref={ref}>
            <CardHeader className="border-b border-slate-200">
                <div className="relative h-52">
                    {product?.thumbnail ? (
                        ...
                        <Image fill src="/imgs/product-placeholder.png" alt="product image" className="w-full object-cover rounded-lg" sizes="(min-width: 1024px) 16vw , 100vw" />
                    )}
                </div>
            </CardHeader>
            <CardContent className="pt-2 text-sm">
                ...
                <p className="mt-1 text-ellipsis truncate">{product?.categories?.length ? product?.categories?.map((category) => category.name).join(", ") : "-"}</p>
                <p className="mt-1">
